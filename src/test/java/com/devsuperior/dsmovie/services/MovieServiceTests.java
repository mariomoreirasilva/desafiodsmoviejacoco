...
public class MovieServiceTests {
	...
	void setup( )throws Exception{
		...
		movieEntity = new MovieFactory().createMovieEntity();
		movieDTO = new MovieFactory().createMovieDTO();
		page = new PageImpl<>(List.of(movieEntity));
		title = "Titulo qualqer";
		existingMovieID = 1L;
		...
		Mockito.doThrow(DatabaseException.class).when(repository).deleteById(nonExistingMovieID);
	...
	public void findAllShouldReturnPagedMovieDTO() {
		...
		Assertions.assertEquals(result.iterator().next().getId(), movieDTO.getId());
	}
	
	@Test
	public void findByIdShouldReturnMovieDTOWhenIdExists() {
		
		movieDTO = service.findById(existingMovieID);
